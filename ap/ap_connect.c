...
void updateAllBeacon(RTMP_ADAPTER *pAd, INT apidx, ULONG FrameLen)
{
	...
	reg_base = pAd->BeaconOffset[apidx];
	...
	reg_base = pAd->BeaconOffset[apidx] + TXWISize;
...
#ifdef RTMP_MAC_USB
	if (IS_USB_INF(pAd)) {
		/* WCNCR00017102:pBeaconSync buf is only available for USB */
		BEACON_SYNC_STRUCT *pBeaconSync = pAd->CommonCfg.pBeaconSync;

		ptr = pMbss->BeaconBuf + pMbss->TimIELocationInBeacon;
		if ((*(ptr + 4)) & 0x01)
...
VOID APUpdateBeaconFrame(RTMP_ADAPTER *pAd, INT apidx)
{
	...
	*ptr = IE_TIM;
	*(ptr + 2) = pAd->ApCfg.DtimCount[apidx];
	*(ptr + 3) = pAd->ApCfg.DtimPeriod[apidx];
	...
	*(ptr + 4) = TimFirst;
	...
	if (pAd->ApCfg.DtimCount[apidx] == 0)
	...
	if ((pMbss->WscControl.WscConfMode != WSC_DISABLE) &&
		...
		/*
		   Non-WPS Windows XP and Vista PCs are unable to determine if a WEP enalbed network
		   is static key based or 802.1X based. If the legacy station gets an EAP-Request/Identity
		   from the AP, it assume the WEP network is 802.1X enabled & will prompt the user for
		   802.1X credentials. If the legacy station doesn't receive anything after sending an
		   EAPOL-Start, it will assume the WEP network is static key based and prompt user
		   for the WEP key. <<from "WPS and Static Key WEP Networks">> A WPS enabled AP
		   should include this IE in the beacon when the AP is hosting a static WEP key network.
		   ...
		   securing_public_wi-fi_hotspots.asp
		 */
		ULONG TempLen = 0;
...
VOID APUpdateAllBeaconFrame(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->ApCfg.DtimCount[MAIN_MBSSID] == 0)
		pAd->ApCfg.DtimCount[MAIN_MBSSID] =
			pAd->ApCfg.DtimPeriod[MAIN_MBSSID] - 1;
	else
		pAd->ApCfg.DtimCount[MAIN_MBSSID] -= 1;
	/* QLOAD ALARM */
#ifdef AP_QLOAD_SUPPORT
