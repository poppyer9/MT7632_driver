...
INT Show_MATTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg);
...
INT RTMPAPPrivIoctlSet(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	UCHAR *buf = NULL;
	UINT16 len = wrq->u.data.length + 1;

	os_alloc_mem(NULL, &buf, len);
	if (!buf)
		return -ENOMEM;

	if (copy_from_user(buf, wrq->u.data.pointer, len)) {
		Status = -EFAULT;
		goto err_free;
	}
	buf[len - 1] = 0;
			 ...
			 INT cmd)
{
	...
	case OID_802_11_DEAUTHENTICATION:
		/* build time check on dst size */
		BUILD_MT_BAD_SZ_M(MLME_QUEUE_ELEM, Msg, MLME_DEAUTH_REQ_STRUCT);
		/* run time check on dst size */
		if (MT_BAD_SZ_M(MLME_QUEUE_ELEM, Msg, MLME_DEAUTH_REQ_STRUCT)) {
			DBGPRINT(RT_DEBUG_ERROR, ("invalid dst copy size\n"));
			Status = -EFAULT;
			break;
		}
		...
		if (wrq->u.data.length != sizeof(MLME_DEAUTH_REQ_STRUCT)) {
			...
			os_alloc_mem(pAd, (UCHAR **) &Elem, sizeof(*Elem));
	...
	case RT_SET_APD_PID:
		{
			...
			size_t len = min(sizeof(apd_pid),
					 (size_t)wrq->u.data.length);
			if (copy_from_user(&apd_pid, wrq->u.data.pointer, len))
				Status = -EFAULT;
			else {
	...
	case RT_SET_DEL_MAC_ENTRY:
		if (copy_from_user(Addr, wrq->u.data.pointer,
				   min(sizeof(Addr),
				       (size_t)wrq->u.data.length))) {
			Status = -EFAULT;
		} else {
...
/*					MacTableDeleteEntry(pAd, pEntry->wcid, Addr); */
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		if (pObj->ioctl_if_type == INT_APCLI) {
			...
			if (pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.AuthMode !=
			    AuthMode) {
				/* Config has changed */
	...
	case OID_802_11_WEP_STATUS:
		if (wrq->u.data.length != sizeof(WepStatus)) {
			Status = -EINVAL;
			break;
		}
	...
	case OID_802_11_SSID:
		if (wrq->u.data.length != sizeof(Ssid)) {
			Status = -EINVAL;
			break;
		}

		Status = copy_from_user(&Ssid, wrq->u.data.pointer,
					sizeof(Ssid));
		if (Status) {
			Status = -EFAULT;
			break;
		}
		if (!Ssid.SsidLength || Ssid.SsidLength > MAX_LEN_OF_SSID) {
			Status = -EINVAL;
			break;
		}

		NdisZeroMemory(pAd->ApCfg.MBSSID[pObj->ioctl_if].Ssid,
			       MAX_LEN_OF_SSID);
		memcpy(pAd->ApCfg.MBSSID[pObj->ioctl_if].Ssid,
		       Ssid.Ssid, Ssid.SsidLength);
		pAd->ApCfg.MBSSID[pObj->ioctl_if].SsidLen = Ssid.SsidLength;
		break;
	...
	case OID_802_11_PASSPHRASE:
		{
			...
			Status = copy_from_user(&tmpPSK, wrq->u.data.pointer,
						min((UINT16)sizeof(tmpPSK),
						    wrq->u.data.length));
			if (Status) {
				Status = -EFAULT;
				break;
			...
			if (countermeasures_enable) {

				{
					...
					for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
						...
						/* happened twice within 60 sec,  AP SENDS disaccociate all
						    associated STAs.  All STA's transition to State 2 */
			   ...
			   INT cmd)
{
		...
		for (i = 0; i < pAd->ScanTab.BssNr; i++) {
			...
			if ((pAd->ScanTab.BssEntry[i].Hidden == 1)) {
				/*
				   ...
				   failed to parse WPA information and then failed to send EAPOl farame.
				 */
				if ((pAd->ApCfg.ApCliTab[ifIndex].wdev.AuthMode >=
				     Ndis802_11AuthModeWPA)
				    && (pMacEntry->PortSecured != WPA_802_1X_PORT_SECURED)) {
	...
	case OID_802_11_BSSID:
		{
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				...
				apcliEn = pAd->ApCfg.ApCliTab[ifIndex].Enable;
				if (!apcliEn) {
					DBGPRINT(RT_DEBUG_TRACE, ("==>OID_802_11_BSSID !apcliEn\n"));
					return FALSE;
				}
	...
	case OID_802_11_WAPI_IE:
		...
		if (wrq->u.data.length != sizeof(WAPI_WIE_STRUCT))
			...
			NdisCopyMemory(wapi_ie.addr, wrq->u.data.pointer,
				       MAC_ADDR_LEN);
			...
			if (pWapiEntry && IS_ENTRY_CLIENT(pWapiEntry)
				...
				NdisCopyMemory(wapi_ie.wie, pWapiEntry->RSN_IE,
					       len);
			}

			if (copy_to_user(wrq->u.data.pointer, &wapi_ie,
					 wrq->u.data.length))
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: copy_to_user() fail\n",
					  __func__));
	...
	case OID_WIFI_TEST_BBP32:
		{
			...
			/* get min: struct bbp32_info + 1(?) dword */
			bc = sizeof(struct bbp32_info) + sizeof(UINT32);
			if (wrq->u.data.length < bc) {
				Status = -EINVAL;
				break;
			}
			...
			Status = copy_from_user(buf, wrq->u.data.pointer,
						wrq->u.data.length);
			if (Status) {
				os_free_mem(NULL, buf);
				buf = NULL;
			...
			/* sanity check: s <= e */
			if (Info->bbp_start > Info->bbp_end) {
				os_free_mem(NULL, buf);
				buf = NULL;
				Status = -EINVAL;
				break;
			...
			/* round up byte count: need 4 bytes if s==e */
			bc = Info->bbp_end - Info->bbp_start;
			bc = (bc / sizeof(UINT32) + 1) * sizeof(UINT32);
			bc += sizeof(struct bbp32_info);
			/* sanity check if dst length enough to copy */
			if (bc > wrq->u.data.length) {
				os_free_mem(NULL, buf);
				buf = NULL;
				Status = -EINVAL;
				break;
			...
			Status = copy_to_user(wrq->u.data.pointer, buf, bc);
			if (Status)
				Status = -EFAULT;
	...
	case OID_WIFI_TEST_RF_BANK:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Info->rf_bank = %x\n", Info->rf_bank));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->rf_start = %x\n", Info->rf_start));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->rf_end = %x\n", Info->rf_end));
	...
	case OID_WIFI_TEST_RF_INDEX:
		{
			...
			/* get min: struct rf_index_info + 1(?) dword */
			bc = sizeof(struct rf_index_info) + sizeof(UINT32);
			if (wrq->u.data.length < bc) {
				Status = -EINVAL;
				break;
			}
			...
			/* round up byte count: need 4 bytes if s==e */
			bc = Info->rf_end - Info->rf_start;
			bc = (bc / sizeof(UINT32) + 1) * sizeof(UINT32);
			bc += sizeof(struct rf_index_info);
			/* sanity check if dst length enough to copy */
			if (bc > wrq->u.data.length) {
				os_free_mem(NULL, buf);
				buf = NULL;
	...
	case OID_WIFI_TEST_MEM_MAP_INFO:
		{
			...
			/* get min: struct mem_map_info + 1(?) dword */
			bc = sizeof(struct mem_map_info) + sizeof(UINT32);
			if (wrq->u.data.length < bc) {
				Status = -EINVAL;
				break;
			}
			...
			/* sanity check: s <= e */
			if (Info->mem_map_start > Info->mem_map_end) {
				os_free_mem(NULL, buf);
				buf = NULL;
				Status = -EINVAL;
				break;
			...
			/* round up byte count: need 4 bytes if s==e */
			bc = Info->mem_map_end - Info->mem_map_start;
			bc = (bc / sizeof(UINT32) + 1) * sizeof(UINT32);
			bc += sizeof(struct mem_map_info);
			/* sanity check if dst length enough to copy */
			if (bc > wrq->u.data.length) {
				os_free_mem(NULL, buf);
				buf = NULL;
	...
	case OID_WIFI_TEST_E2P:
		{
			...
			/* get min: struct e2p_info + 1(?) UINT16 */
			bc = sizeof(struct e2p_info) + sizeof(UINT16);
			if (wrq->u.data.length < bc) {
				Status = -EINVAL;
				break;
			}
			...
			/* sanity check: s <= e */
			if (Info->e2p_start > Info->e2p_end) {
				os_free_mem(NULL, buf);
				buf = NULL;
				Status = -EINVAL;
				break;
			...
			/* round up byte count: need 2 bytes if s==e */
			bc = Info->e2p_end - Info->e2p_start;
			bc = (bc / sizeof(UINT16) + 1) * sizeof(UINT16);
			bc += sizeof(struct e2p_info);
			/* sanity check if dst length enough to copy */
			if (bc > wrq->u.data.length) {
				os_free_mem(NULL, buf);
				buf = NULL;
	...
	case OID_WIFI_TEST_MAC:
		{
			...
			/* get min: struct mem_map_info + 1(?) dword */
			bc = sizeof(struct mac_info) + sizeof(UINT32);
			if (wrq->u.data.length < bc) {
				Status = -EINVAL;
				break;
			}
			...
			/* sanity check: s <= e */
			if (Info->mac_start > Info->mac_end) {
				os_free_mem(NULL, buf);
				buf = NULL;
				Status = -EINVAL;
				break;
			...
			/* round up byte count: need 4 bytes if s==e */
			bc = Info->mac_end - Info->mac_start;
			bc = (bc / sizeof(UINT32) + 1) * sizeof(UINT32);
			bc += sizeof(struct mac_info);
			/* sanity check if dst length enough to copy */
			if (bc > wrq->u.data.length) {
				os_free_mem(NULL, buf);
				buf = NULL;
...
INT Set_2G_256QAM_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	...
	pAdapter->CommonCfg.b256QAM_2G = (UCHAR)res;
	return success;
...
INT Set_ACKQ_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	...
	pAdapter->CommonCfg.ACKQEN = (BOOLEAN)res;
...
INT Set_ACKWait_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	...
	pAdapter->CommonCfg.AckWaitTime = (UCHAR)res;
...
INT Set_ACKLen_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	...
	pAdapter->CommonCfg.Acklen = (UCHAR)res;
...
INT Set_TxRate_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	pAd->ApCfg.MBSSID[pObj->ioctl_if].DesiredRatesIndex = (UCHAR)res;
...
INT Set_DtimPeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DtimPeriod = (USHORT)res;
	if ((DtimPeriod >= 1) && (DtimPeriod <= 255)) {
		pAd->ApCfg.DtimPeriod[MAIN_MBSSID] = DtimPeriod;
		success = TRUE;
	} else
		success = FALSE;
...
INT Set_HideSSID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pAd->ApCfg.MBSSID[pObj->ioctl_if].hidden_ssid != res)
		pAd->ApCfg.MBSSID[pObj->ioctl_if].hidden_ssid = res;
...
INT Set_ACLShowAll_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("=============== Access Control Policy ===============\n"));
	DBGPRINT(RT_DEBUG_TRACE, ("Policy is %ld : ", acl.Policy));
	DBGPRINT(RT_DEBUG_TRACE, ("%s\n", pACL_PolicyMessage[acl.Policy]));
	/* Dump the entry in the list one by one */
	DBGPRINT(RT_DEBUG_TRACE, ("===============  Access Control List  ===============\n"));
	for (i = 0; i < acl.Num; i++) {
		DBGPRINT(RT_DEBUG_TRACE, ("Entry #%02d: ", i + 1));
		for (j = 0; j < MAC_ADDR_LEN; j++)
...
INT Show_StaCount_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry))
			...
			DBGPRINT(RT_DEBUG_TRACE, ("%-12ld", (ULONG) pEntry->TxPackets.QuadPart));
			DBGPRINT(RT_DEBUG_TRACE, ("%-12ld", (ULONG) pEntry->RxPackets.QuadPart));
			DBGPRINT(RT_DEBUG_TRACE, ("%-12ld", (ULONG) pEntry->TxBytes));
			DBGPRINT(RT_DEBUG_TRACE, ("%-12ld", (ULONG) pEntry->RxBytes));
			DBGPRINT(RT_DEBUG_TRACE, ("\n"));
		}
	}
...
INT Show_StaSecurityInfo_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		wdev = &pAd->ApCfg.MBSSID[apidx].wdev;
		DBGPRINT(RT_DEBUG_TRACE,
		(" BSS(%d) AuthMode(%d)=%s, WepStatus(%d)=%s, GroupWepStatus(%d)=%s, WPAMixPairCipher(0x%02X)\n",
		apidx, wdev->AuthMode, GetAuthMode(wdev->AuthMode), wdev->WepStatus,
		GetEncryptType(wdev->WepStatus), wdev->GroupKeyWepStatus,
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pEntry && IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) {
			DBGPRINT(RT_DEBUG_TRACE, ("%02X:%02X:%02X:%02X:%02X:%02X  ", PRINT_MAC(pEntry->Addr)));
			DBGPRINT(RT_DEBUG_TRACE, ("%-4d", (int)pEntry->Aid));
			DBGPRINT(RT_DEBUG_TRACE, ("%-4d", (int)pEntry->apidx));
			DBGPRINT(RT_DEBUG_TRACE, ("%-15s", GetAuthMode(pEntry->AuthMode)));
			DBGPRINT(RT_DEBUG_TRACE, ("%-12s", GetEncryptType(pEntry->WepStatus)));
			DBGPRINT(RT_DEBUG_TRACE, ("\n"));
		}
	}
...
INT Show_BaTable_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%02X:%02X:%02X:%02X:%02X:%02X (Aid = %d) (%s) -\n",
		       PRINT_MAC(pEntry->Addr), pEntry->Aid, tmpBuf));
		DBGPRINT(RT_DEBUG_TRACE, ("[Recipient]\n"));
		for (j = 0; j < NUM_OF_TID; j++) {
			if (pEntry->BARecWcidArray[j] != 0) {
				pRecBAEntry = &pAd->BATable.BARecEntry[pEntry->BARecWcidArray[j]];
				DBGPRINT(RT_DEBUG_TRACE, ("TID=%d, BAWinSize=%d, LastIndSeq=%d, ReorderingPkts=%d\n",
				       j, pRecBAEntry->BAWinSize, pRecBAEntry->LastIndSeq, pRecBAEntry->list.qlen));
			}
		}
		DBGPRINT(RT_DEBUG_TRACE, ("\n"));
		DBGPRINT(RT_DEBUG_TRACE, ("[Originator]\n"));
		for (j = 0; j < NUM_OF_TID; j++) {
			if (pEntry->BAOriWcidArray[j] != 0) {
				pOriBAEntry = &pAd->BATable.BAOriEntry[pEntry->BAOriWcidArray[j]];
				DBGPRINT(RT_DEBUG_TRACE, ("TID=%d, BAWinSize=%d, StartSeq=%d, CurTxSeq=%d\n", j,
				       pOriBAEntry->BAWinSize, pOriBAEntry->Sequence, pEntry->TxSeq[j]));
			}
		}
		DBGPRINT(RT_DEBUG_TRACE, ("\n\n"));
	}
...
INT Show_Sat_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("TransmitCountFromOS = %d\n", pAd->WlanCounters.TransmitCountFrmOs.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedFragmentCount = %lld\n",
			pAd->WlanCounters.TransmittedFragmentCount.u.LowPart +
			pAd->WlanCounters.MulticastTransmittedFrameCount.QuadPart));
	DBGPRINT(RT_DEBUG_TRACE, ("MulticastTransmittedFrameCount = %d\n",
			pAd->WlanCounters.MulticastTransmittedFrameCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("FailedCount = %d\n", pAd->WlanCounters.FailedCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("RetryCount = %d\n", pAd->WlanCounters.RetryCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("MultipleRetryCount = %d\n", pAd->WlanCounters.MultipleRetryCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("RTSSuccessCount = %d\n", pAd->WlanCounters.RTSSuccessCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("RTSFailureCount = %d\n", pAd->WlanCounters.RTSFailureCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("ACKFailureCount = %d\n", pAd->WlanCounters.ACKFailureCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("FrameDuplicateCount = %d\n", pAd->WlanCounters.FrameDuplicateCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("ReceivedFragmentCount = %d\n", pAd->WlanCounters.ReceivedFragmentCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("MulticastReceivedFrameCount = %d\n",
		pAd->WlanCounters.MulticastReceivedFrameCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("Rx drop due to out of resource  = %ld\n", (ULONG) pAd->Counters8023.RxNoBuffer));
#ifdef DBG
	DBGPRINT(RT_DEBUG_TRACE, ("RealFcsErrCount = %d\n", pAd->RalinkCounters.RealFcsErrCount.u.LowPart));
#else
	DBGPRINT(RT_DEBUG_TRACE, ("FCSErrorCount = %d\n", pAd->WlanCounters.FCSErrorCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("FrameDuplicateCount.LowPart = %d\n",
		pAd->WlanCounters.FrameDuplicateCount.u.LowPart / 100));
#endif /* endif */
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedFrameCount = %d\n",
		pAd->WlanCounters.TransmittedFragmentCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("WEPUndecryptableCount = %d\n", pAd->WlanCounters.WEPUndecryptableCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("\n===Some 11n statistics variables:\n"));
	/* Some 11n statistics variables */
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedAMSDUCount = %ld\n",
		(ULONG) pAd->RalinkCounters.TransmittedAMSDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedOctetsInAMSDU = %ld\n",
		(ULONG) pAd->RalinkCounters.TransmittedOctetsInAMSDU.QuadPart));
	DBGPRINT(RT_DEBUG_TRACE, ("ReceivedAMSDUCount = %ld\n",
		(ULONG) pAd->RalinkCounters.ReceivedAMSDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("ReceivedOctesInAMSDUCount = %ld\n",
		(ULONG) pAd->RalinkCounters.ReceivedOctesInAMSDUCount.QuadPart));
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedAMPDUCount = %ld\n",
		(ULONG) pAd->RalinkCounters.TransmittedAMPDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedMPDUsInAMPDUCount = %ld\n",
	       (ULONG) pAd->RalinkCounters.TransmittedMPDUsInAMPDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("TransmittedOctetsInAMPDUCount = %ld\n",
	       (ULONG) pAd->RalinkCounters.TransmittedOctetsInAMPDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("MPDUInReceivedAMPDUCount = %ld\n",
		(ULONG) pAd->RalinkCounters.MPDUInReceivedAMPDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_TRACE, ("fAnyStaFortyIntolerant=%d\n", pAd->MacTab.fAnyStaFortyIntolerant));

	{
		int apidx;

		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			DBGPRINT(RT_DEBUG_TRACE, ("-- IF-ra%d --\n", apidx));
			DBGPRINT(RT_DEBUG_TRACE, ("Packets Received = %ld\n",
				(ULONG) pAd->ApCfg.MBSSID[apidx].RxCount));
			DBGPRINT(RT_DEBUG_TRACE, ("Packets Sent = %ld\n", (ULONG) pAd->ApCfg.MBSSID[apidx].TxCount));
			DBGPRINT(RT_DEBUG_TRACE, ("Bytes Received = %ld\n",
				(ULONG) pAd->ApCfg.MBSSID[apidx].ReceivedByteCount));
			DBGPRINT(RT_DEBUG_TRACE, ("Byte Sent = %ld\n",
				(ULONG) pAd->ApCfg.MBSSID[apidx].TransmittedByteCount));
			DBGPRINT(RT_DEBUG_TRACE, ("Error Packets Received = %ld\n",
				(ULONG) pAd->ApCfg.MBSSID[apidx].RxErrorCount));
			DBGPRINT(RT_DEBUG_TRACE, ("Drop Received Packets = %ld\n",
			...
			DBGPRINT(RT_DEBUG_TRACE, ("-- IF-ra%d end --\n", apidx));
		}
	}
				...
				for (j = maxMcs; j >= 0; j--) {
					if ((pEntry->TXMCSExpected[j] != 0)
						...
						DBGPRINT(RT_DEBUG_TRACE,
						     ...
						     pEntry->TXMCSExpected[j] ? (100 * pEntry->TXMCSSuccessful[j])
						     / pEntry->TXMCSExpected[j] : 0, pEntry->TXMCSFailed[j]));
						for (k = maxMcs; k >= 0; k--) {
							if (pEntry->TXMCSAutoFallBack[j][k] != 0) {
								DBGPRINT(RT_DEBUG_TRACE,
									("\t\t\tAutoMCS[%02d]: %u (%d%%)\n",
								     k, pEntry->TXMCSAutoFallBack[j]
								     [k], (100 * pEntry->TXMCSAutoFallBack[j]
									 [k]) / pEntry->TXMCSExpected[j]));
							}
						}
					}
				}
...
INT Show_MATTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Default BroadCast Address=%02x:%02x:%02x:%02x:%02x:%02x!\n", BROADCAST_ADDR[0],
	       BROADCAST_ADDR[1], BROADCAST_ADDR[2], BROADCAST_ADDR[3], BROADCAST_ADDR[4], BROADCAST_ADDR[5]));
	return TRUE;
}
#endif /* MAT_SUPPORT */
	...
	if (pAd->ApCfg.MBSSID[apidx].wdev.AuthMode >= Ndis802_11AuthModeWPA) {
		if ((pKey->KeyLength == 32) || (pKey->KeyLength == 64)) {
			...
			if (pEntry != NULL) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
	("RTMPIoctlAddWPAKey-IF(ra%d) : Add PMK=%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x....\n",
					  apidx, pAd->ApCfg.MBSSID[apidx].PMK[0],
...
VOID RTMPIoctlAddPMKIDCache(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	("WPA2(pre-auth):(%02x:%02x:%02x:%02x:%02x:%02x)Calc PMKID=%02x:%02x:%02x:%02x:%02x:%02x\n",
				  ...
				  pKey->addr[4], pKey->addr[5], digest[0], digest[1], digest[2],
...
static VOID RTMPAPIoctlBBP32(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	const size_t mtotal = MAX_BBP_MSG_SIZE * 2 + arg_total + 12;
	...
	if (mpool + mtotal < arg + argLen + 1) {
		DBGPRINT(RT_DEBUG_WARN,
			 ("%s arg too long, arg 0x%p, len %d\n",
			  __func__, arg, argLen));
		DBGPRINT(RT_DEBUG_WARN, ("mpoll 0x%p, len %zu\n",
			 mpool, mtotal));
		goto err_free;
	}
				...
				if (!value || !*value) {	/*Read */
					if (sscanf(this_char, "%d", &(bbpId)) == 1) {
						if (bbpId <= pAdapter->chipCap.MaxNumOfBbpId) {
							...
							/*
							   ...
							   We must access BBP directly. For RT2870 ATE mode,
					...
					if ((sscanf(this_char, "%d", &(bbpId)) == 1)
						...
						if (bbpId <= pAdapter->chipCap.MaxNumOfBbpId) {
							...
							{
					/* according to Andy, Gary, David require. */
					/* the command bbp shall read/write BBP register directly for debug. */
					/* BBP_IO_READ8_BY_REG_ID(pAdapter, bbpId, &regBBP); */
								RTMP_BBP_IO_READ8_BY_REG_ID
								    (pAdapter, bbpId, &regBBP);
...
VOID RTMPAPIoctlMAC(IN RTMP_ADAPTER *pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	INT argLen;
	const size_t arg_total = 256;
	const size_t mtotal = 4096 + arg_total + 12;
...
VOID RTMPAPIoctlE2PROM(IN PRTMP_ADAPTER pAdapter, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	NdisZeroMemory(mpool, mtotal);
	if (argLen > 1) { /* If no parameter, dump all e2p. */
		if (copy_from_user(arg, wrq->u.data.pointer, argLen)) {
			DBGPRINT(RT_DEBUG_WARN,
				 ("copy_from_user fail, len %d\n", argLen));
			goto done;
		...
		if (pAd->MacTab.Size > 0) {
			...
			for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				...
				if (IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) {
					...
					pEntry->wdev->if_addr[0], pEntry->wdev->if_addr[1],
					pEntry->wdev->if_addr[2],  pEntry->wdev->if_addr[3],
					pEntry->wdev->if_addr[4],  pEntry->wdev->if_addr[5]); */
...
#define IsZero(c) ('0' == (c) ? TRUE:FALSE)
	PinCode = res;	/* When PinCode is 03571361, return value is 3571361. */
...
INT Set_RADIUS_Port_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (count = 0, macptr = rstrtok(arg, ";"); (macptr && count < MAX_RADIUS_SRV_NUM);
		...
		if (srv_cnt < pAd->ApCfg.MBSSID[apidx].radius_srv_num) {
			...
			ret = os_strtol(macptr, 10, &res);
			if (ret < 0)
				return FALSE;

			pSrvInfo->radius_port = (UINT32) res;
			srv_cnt++;
...
INT Set_McastMcs(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (Mcs > 15)
		DBGPRINT(RT_DEBUG_TRACE, ("Mcs must in range of 0 to 15\n"));
...
INT Set_OBSSScanParam_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		ret = os_strtol(thisChar, 10, &res);
		if (ret < 0)
			return FALSE;

		ObssScanValue = (INT) res;
		switch (Idx) {
		case 0:
...
INT ApCfg_Set_MaxStaNum_Proc(IN PRTMP_ADAPTER pAd, IN INT apidx, IN PSTRING arg)
{
	...
	pAd->ApCfg.MBSSID[apidx].MaxStaNum = (UCHAR) res;
			...
			IN INT cmd, IN USHORT subcmd, IN VOID *pData, IN ULONG Data)
{
	...
	case CMD_RT_PRIV_IOCTL:
		...
		if (subcmd & OID_GET_SET_TOGGLE)
			Status = RTMPAPSetInformation(pAd, wrq, subcmd);
		else
			Status = RTMPAPQueryInformation(pAd, wrq, subcmd);
