...
{                                                                                       \
	...
	NdisMoveMemory((_pAd)->MacTab.Content[BSSID_WCID].HTCapability.MCSSet, \
	(_pAd)->StaActive.SupportedPhyInfo.MCSSet, sizeof(UCHAR) * 16);\
}
...
#define GET_LNA_GAIN(_pAd)	((_pAd->LatchRfRegs.Channel <= 14) ? \
	(_pAd->BLNAGain) : ((_pAd->LatchRfRegs.Channel <= 64) ? \
	(_pAd->ALNAGain0) : ((_pAd->LatchRfRegs.Channel <= 128) ? \
	(_pAd->ALNAGain1) : (_pAd->ALNAGain2))))
...
#define LEAP_CCKM_ON(_p)   ((((_p)->StaCfg.LeapAuthMode) == CISCO_AuthModeLEAP) && \
							((_p)->StaCfg.LeapAuthInfo.CCKM == TRUE))
...
{                                                                                       \
	...
	MlmeEnqueueForRecv(_pAd, Wcid, High32TSF, Low32TSF, (UCHAR)_Rssi0, (UCHAR)_Rssi1, \
	(UCHAR)_Rssi2, _FrameSize, _pFrame, (UCHAR)_MinSNR, _OpMode);   \
}
#endif /* RTMP_MAC_USB */
...
typedef struct _CHANNEL_TX_POWER {
	...
	UCHAR Channel;
	BOOLEAN bEffectedChannel;	/* For BW 40 operating in 2.4GHz , the "effected channel"
					*  is the channel that is covered in 40Mhz.
								*/
	CHAR Power;
	CHAR Power2;
#ifdef DOT11N_SS3_SUPPORT
...
typedef struct GNU_PACKED _OID_BA_REC_ENTRY {
	UCHAR MACAddr[MAC_ADDR_LEN];
	UCHAR BaBitmap;		/* if (BaBitmap&(1<<TID)), this session with{MACAddr, TID}exists,
						* so read BufSize[TID] for BufferSize
						*/
	UCHAR rsv;
	UCHAR BufSize[8];
...
typedef struct GNU_PACKED _OID_BA_ORI_ENTRY {
	UCHAR MACAddr[MAC_ADDR_LEN];
	UCHAR BaBitmap;		/* if (BaBitmap&(1<<TID)), this session with{MACAddr, TID}exists,
	* so read BufSize[TID] for BufferSize, read ORI_BA_Status[TID] for status
						*/
	UCHAR rsv;
	UCHAR BufSize[8];
...
typedef union _BACAP_STRUC {
	...
	struct {
		UINT32:4;
		UINT32 b2040CoexistScanSup:1;	/*As Sta, support do 2040 coexistence scan for AP.
						* As Ap, support monitor trigger event to check
						* if can use BW 40MHz.
										*/
		...
		/* 0: DELAY_BA 1:IMMED_BA  (//BA Policy subfiled value in ADDBA frame)
		* 2:BA-not use
		*/
		UINT32 Policy:2;
...
/* This structure is for all 802.11n card InterOptibilityTest action. Reset all Num every n second.
* (Details see MLMEPeriodic)
*/
...
typedef struct _COMMON_CONFIG {
...
#endif				/* SINGLE_SKU_V2 */
	UCHAR CountryRegion;	/* Enum of country region, 0:FCC, 1:IC, 2:ETSI, 3:SPAIN,
							* 4:France, 5:MKK, 6:MKK1, 7:Israel
							*/
	...
	REG_TRANSMIT_SETTING RegTransmitSetting; /*registry transmit setting. this is for
			* reading registry setting only. not useful.
											 */
	...
	BOOLEAN bEnableTxBurst;	/* 1: enble TX PACKET BURST (when BA is established or AP is not
				* a legacy WMM AP), 0: disable TX PACKET BURST
							*/
	BOOLEAN bAggregationCapable;	/* 1: enable TX aggregation when the peer supports it */
	BOOLEAN bPiggyBackCapable;	/* 1: enable TX piggy-back according MAC's version */
	BOOLEAN bIEEE80211H;	/* 1: enable IEEE802.11h spec. */
	/* 1:enable passive scan, 0:skip scanning on DFS channel */
	BOOLEAN bIEEE80211H_PASSIVE_SCAN;
	...
	NEW_EXT_CHAN_IE NewExtChanOffset;	/*7.3.2.20A, 1 if extension channel is above the
		* control channel, 3 if below, 0 if not present
										*/

	EXT_CAP_INFO_ELEMENT ExtCapIE;	/* this is the extended capibility IE appreed in MGMT
		* frames. Doesn't need to update once set in Init.
									*/
	...
	ULONG Dot11BssWidthChanTranDelay;	/* multiple of (Dot11BssWidthTriggerScanInt *
						* Dot11BssWidthChanTranDelayFactor)
										*/
	ULONG CountDownCtr;	/* CountDown Counter from (Dot11BssWidthTriggerScanInt *
						* Dot11BssWidthChanTranDelayFactor)
						*/
...
typedef struct _CFG_TDLS_STRUCT {
	...
	UINT8 NeedToTearDown;	/* recving frDs=0 toDS=0, but entry is gone, send teardown to
				* peer within TDLS_AUTO_TEARDOWN_WRONG_PATH_THRESHOLD pkts
							*/

	/* Channel Switch */
	UCHAR CHSWPeerMacAddr[MAC_ADDR_LEN];
	BOOLEAN bDoingPeriodChannelSwitch;
	BOOLEAN bDoingChannelSwitch;
	...
	BOOLEAN IsentCHSW;
#ifdef CERTIFICATION_SIGMA_SUPPORT
	RALINK_TIMER_STRUCT BeaconSyncTimer;	/* Use to sync TBTT */
...
#define MONITOR_FLAG_11N_SNIFFER		0x01
	...
	/* stay time at op channel for ImprovedScan*/
	UINT32 OpChannelTime;
	/************************************/
...
/* UCHAR DeviceName[P2P_DEVICE_NAME_LEN]; */
	...
	UINT32 HeaderBuf[32];	/* (total 128B) TempBuffer for TX_INFO + TX_WI + 802.11 Header +
							* padding + AMSDU SubHeader + LLC/SNAP
							*/
...
/*==================================================== */
	...
	USHORT BARecWcidArray[NUM_OF_TID];	/* The mapping wcid of recipient session.
						* if RXBAbitmap bit is masked
										*/
	USHORT BAOriWcidArray[NUM_OF_TID];	/* The mapping wcid of originator session.
						* if TXBAbitmap bit is masked
										*/
	USHORT BAOriSequence[NUM_OF_TID];	/* The mapping wcid of originator session.
						* if TXBAbitmap bit is masked
										*/
...
typedef struct _MAC_TABLE {
	...
	BOOLEAN fAnyStationBadAtheros;	/* Check if any Station is atheros 802.11n Chip.
...
#endif				/*CONFIG_SNIFFER_SUPPORT */
#ifdef APCLI_SUPPORT
	/* Number of  ApClient interface which was running. value from 0 to MAX_APCLI_INTERFACE */
	UCHAR ApCliInfRunned;
	UINT8 ApCliNum;
...
#define DEFLAUT_PARTIAL_SCAN_BREAK_TIME	4  /* Period of partial scanning: unit: 100ms
					    * Must be move to other place
						*/
	...
	UCHAR DtimCount[HW_BEACON_MAX_NUM];	/* 0.. DtimPeriod-1 */
	UCHAR DtimPeriod[HW_BEACON_MAX_NUM];	/* default = 3 */
	UCHAR ErpIeContent;
	...
	UCHAR PreAuthifname[HW_BEACON_MAX_NUM][IFNAMSIZ]; /* indicate as the binding
			* interface for WPA2 Pre-authentication.
												*/
...
#define WOW_MAX_PORT_NUM_TCPV4		(20)
...
#define WMT_GPIO_CONFIG_OPCODE		0x18
#define WMT_GPIO_CONFIG_LEN			0x34
#define WMT_WOW_GPIO				0x05
#define WMT_WOBLE_GPIO				0x13
#define WMT_GPIO_CONFIG_WIFI		0x00
#define WOW_TCP_PORT_CNT_LEN		0x08
#define WOW_UDP_PORT_CNT_LEN		0x08
...
typedef struct _WOW_CFG_STRUCT {
	...
	BOOLEAN	bWmtGPIOCmd;	/* Enable Wmt GPIO command setting */
	BOOLEAN	bAlwaysTrigger;	/* Enable wow always trigger wakeup*/
	BOOLEAN	bWakeonportEnable;	/* Enable wow wake-on port */
	BOOLEAN	bWoWRunning;  /* Now WOW is running now */
	BOOLEAN	bWmtWiFiHighActive; /* GPIO high/low active (default GPIO5) */
	BOOLEAN	bWmtBTHighActive; /* GPIO high/low active (default GPIO19) */
	BOOLEAN	bWmtWiFiWake1Toggle; /* enable wifi GPIO toggle for WOW */
	BOOLEAN	bWmtWiFiWake2Toggle; /* enable wifi GPIO toggle for WOBLE */
	BOOLEAN	bWmtWiFiWake3Toggle; /* enable wifi GPIO toggle for WOBLE ambilight */
	BOOLEAN	bWmtBTWake1Toggle; /* enable bt GPIO toggle for WOW */
	BOOLEAN	bWmtBTWake2Toggle; /* enable bt GPIO toggle for WOBLE */
	BOOLEAN	bWmtBTWake3Toggle; /* enable bt GPIO toggle for WOBLE ambilight */
	BOOLEAN bAdvEnable;		/* Enable advanced WOW function */
	UINT8 nAdvSleepTime;	/* Advanced WOW sleep cycle time */
	...
	UINT8 nWmtGPIOMode;	/* GPIO two-pin/share/dedicated mode */
	UINT16 nWmtWiFiWake1HoldTime; /* wifi GPIO pulse hold time for wow */
	UINT16 nWmtWiFiWake2HoldTime; /* wifi GPIO pulse hold time for WOBLE */
	UINT16 nWmtWiFiWake3HoldTime; /* wifi GPIO pulse hold time for WOBLE ambilight */
	UINT16 nWmtBTWake1HoldTime; /* bt GPIO pulse hold time for wow */
	UINT16 nWmtBTWake2HoldTime; /* bt GPIO pulse hold time for WOBLE */
	UINT16 nWmtBTWake3HoldTime; /* bt GPIO pulse hold time for WOBLE ambilight */
	...
	UINT32 WakeupReason;		/* Store wakeup reason */
#ifdef RT_CFG80211_SUPPORT
...
enum {
	...
	WOW_IPV4_TCP_SYNC,
	WOW_IPV6_TCP_SYNC,
	WOW_PKT_TCP_PORT_ARRAY = 14,
	WOW_PKT_UDP_PORT_ARRAY = 15
};
...
struct NEW_WOW_ALWAYS_TRIGGER_STRUCT {
	UINT32 Parameter;
...
/* It needs pack 1 bytes, otherwise system can
auto aglignment leading to send the wrong data*/
#pragma pack(1)
struct NEW_WMT_WAKEUP_GPIO_STRUCT {
	UINT16	HCI_OPCode;
	UINT8	HCI_Length;
	UINT8	WMT_Direct_Type;
	UINT8	WMT_OPCode;
	UINT16	WMT_Length;
	UCHAR	GPIO_Pin_Mode;
	UCHAR	Trigger;
	UINT16	Reserve1;
	UCHAR	WoW_Pin_Num;
	UCHAR	WoW_Initial_Level;
	UINT16	WiFi_Reserve1;
	UINT16	WiFi_Level_Wake1_Time1;
	UINT16	WiFi_Level_Wake1_Time2;
	UINT16	WiFi_Level_Wake1_Time3;
	UINT16	WiFi_Level_Wake2_Time1;
	UINT16	WiFi_Level_Wake2_Time2;
	UINT16	WiFi_Level_Wake2_Time3;
	UINT16	WiFi_Level_Wake3_Time1;
	UINT16	WiFi_Level_Wake3_Time2;
	UINT16	WiFi_Level_Wake3_Time3;
	UINT16	WiFi_Reserve2;
	UCHAR	WoBT_Pin_Num;
	UCHAR	WoBT_Initial_Level;
	UINT16	BT_Reserve1;
	UINT16	BT_Level_Wake1_Time1;
	UINT16	BT_Level_Wake1_Time2;
	UINT16	BT_Level_Wake1_Time3;
	UINT16	BT_Level_Wake2_Time1;
	UINT16	BT_Level_Wake2_Time2;
	UINT16	BT_Level_Wake2_Time3;
	UINT16	BT_Level_Wake3_Time1;
	UINT16	BT_Level_Wake3_Time2;
	UINT16	BT_Level_Wake3_Time3;
	UINT16	BT_Reserve2;
};
#pragma pack()

struct NEW_WOW_BUF_SEQ_NUM_CFG_STRUCT {
	UINT32	Config_Type;
};

#ifdef WOW_BUF_PKT_SUPPORT
struct NEW_WOW_BUF_PKT_STRUCT {
	UINT32	Config_Type;
};
...
#ifdef RTMP_MAC_USB
struct usb_dp_ctrl {
	UINT8			nUsb2DisconMode;  /* Select USB disconnect mode */
	UINT8			nGPIONotifyMode;  /* Select GPIO notify interrupt mode */
	UINT8			nGPIONotifyPin;  /* Select GPIO notify pin	*/
};
#endif
...
typedef struct _CFG80211_CONTROL {
	...
	UCHAR APCurrentAddress[MAC_ADDR_LEN];
#ifdef RT_CFG80211_AP_SUPPORT
	BOOLEAN cfg_ap_is_working;
	BOOLEAN ap_mac_is_changed;
...
#define CFG80211_MULTI_AP_NUM 1
	PNET_DEV multi_ap_net_dev;
	BOOLEAN flg_cfg_multi_ap_init;
	...
	ULONG P2POpStatusFlags;	/* P2P_CLI_UP / P2P_GO_UP / P2P_IF_UP */
...
#endif				/* CUSTOMIZED_FEATURE_SUPPORT */
#ifdef RT_CFG80211_AP_SUPPORT
	PNET_DEV cfg_ap_net_dev;
#endif /* RT_CFG80211_AP_SUPPORT */

	BOOLEAN flg_cfg_ap_init;
...
struct _RTMP_ADAPTER {
	struct os_cookie *OS_Cookie; /* os specific structure */
	PNET_DEV net_dev;
...
/*********************************************************/
	...
	ULONG NextRxBulkInPosition;	/* Want to contatenate 2 URB buffer while 1st is bulkin
					* failed URB.This Position is 1st URB TransferLength.
								*/
...
/***********************************************************/
	...
	EEPROM_ANTENNA_STRUC Antenna;	/* Since ANtenna definition is different for a & g.
					* We need to save it for future reference.
									*/
	...
	UCHAR BbpWriteLatch[MAX_BBP_ID + 1];	/* record last BBP register value written
						* via BBP_IO_WRITE/BBP_IO_WRITE_VY_REG_ID
											*/
	UCHAR Bbp94;
...
/*=======Common=========== */
	...
	MAC_TABLE MacTab;	/* ASIC on-chip WCID entry table.  At TX,
				* ASIC always use key according to this on-chip table.
						*/
...
#define ED_STAT_CNT 20
	...
	BOOLEAN do_cap;		/* 1: start to do cap, if auto, will triggered depends on trigger
						* condition, if manual, start immediately
						*/
	...
	BOOLEAN bVendorIEEnable;
	PUCHAR pVendorIE;
	UCHAR vendorIELen;
	UINT32 vendorIELocation;
};
...
typedef struct _TX_BLK {
	 ...
	 */
	UCHAR HeaderBuf[128];	/* TempBuffer for TX_INFO + TX_WI + TSO_INFO + 802.11 Header
							* + padding + AMSDU SubHeader + LLC/SNAP
							*/
#else
...
#define fTX_bRtsRequired		0x0001	/* Indicate if need send RTS frame for protection.
						* Not used in RT2860/RT2870.
										*/
#define fTX_bAckRequired		0x0002	/* the packet need ack response */
#define fTX_bPiggyBack			0x0004	/* Legacy device use Piggback or not */
#define fTX_bHTRate				0x0008	/* allow to use HT rate */
#define fTX_bForceNonQoS		0x0010	/* force to transmit frame without WMM-QoS in HT mode */
#define fTX_bAllowFrag			0x0020	/* allow to fragment the packet, A-MPDU, A-MSDU,
						* A-Ralink is not allowed to fragment
										*/
...
#ifdef WAPI_SUPPORT
#define	fTX_bWPIDataFrame		0x8000	/* indicate this packet is an WPI data frame,
						it need to be encrypted by software
						*/
#endif /* WAPI_SUPPORT */
...
VOID RTMPWriteTxWI(IN RTMP_ADAPTER *pAd, IN TXWI_STRUC *pTxWI,
	IN BOOLEAN FRAG, IN BOOLEAN CFACK, IN BOOLEAN InsTimestamp,
	IN BOOLEAN AMPDU, IN BOOLEAN Ack, IN BOOLEAN NSeq,	/* HW new a sequence. */
		   IN UCHAR BASize,
		   IN UCHAR WCID,
		   IN ULONG Length,
		   IN UCHAR PID,
...
VOID RTMPWriteTxWI_Data(IN RTMP_ADAPTER *pAd, INOUT TXWI_STRUC *pTxWI, IN TX_BLK * pTxBlk);

VOID RTMPWriteTxWI_Cache(IN RTMP_ADAPTER *pAd, INOUT TXWI_STRUC *pTxWI, IN TX_BLK * pTxBlk);
...
VOID AsicSetTxTSFCompensation(PRTMP_ADAPTER pAd, u32 val);
...
#ifdef CONFIG_AP_SUPPORT
INT AsicSetRepeaterMode(PRTMP_ADAPTER pAd, BOOLEAN bEnable);
VOID AsicSetRepeaterExtAddr(PRTMP_ADAPTER pAd, PUCHAR pBssid);
...
BOOLEAN RTMPRcvFrameDLSCheck(IN RTMP_ADAPTER *pAd,
			     IN PHEADER_802_11 pHeader, IN ULONG Len, IN RXINFO_STRUC *pRxInfo);
...
BOOLEAN PeerAssocRspSanity(IN RTMP_ADAPTER *pAd, IN VOID *pMsg,
...
VOID WscSendMessage(IN RTMP_ADAPTER *pAd, IN UCHAR OpCode,
	IN PUCHAR pData, IN INT Len, IN PWSC_CTRL pWscControl,
...
#define  RTMP_AP_802_11_REMOVE_LLC_AND_CONVERT_TO_802_3(_pRxBlk, _pHeader802_3)	   \
	do {                                                                       \
		...
		if (RX_BLK_TEST_FLAG(_pRxBlk, fRX_WDS) ||                          \
			RX_BLK_TEST_FLAG(_pRxBlk, fRX_MESH)) {		           \
...
VOID Update_Rssi_Sample(IN RTMP_ADAPTER *pAd, IN RSSI_SAMPLE *pRssi, IN RXWI_STRUC *pRxWI);
...
#ifdef CFG_TDLS_SUPPORT
INT Set_CfgTdlsChannelSwitchRequest_Proc(PRTMP_ADAPTER pAd, PSTRING arg);
INT Set_CfgTdlsChannelSwitchBaseStayTime_Proc(PRTMP_ADAPTER pAd, PSTRING arg);
INT Set_CfgTdlsChannelSwitchBW_Proc(PRTMP_ADAPTER pAd, PSTRING arg);
INT Set_TdlsDisableChannelSwitchProc(PRTMP_ADAPTER pAd, PSTRING arg);
#endif /* CFG_TDLS_SUPPORT */
...
VOID RtmpPrepareHwNullFrame(RTMP_ADAPTER *pAd, PMAC_TABLE_ENTRY pEntry,
			    BOOLEAN bQosNull, BOOLEAN bEOSP, UCHAR OldUP,
			    UCHAR OpMode, UCHAR PwrMgmt, BOOLEAN bWaitACK,
			    UCHAR Index);
...
INT RtmpOSNotifyRawData(IN PNET_DEV pNetDev, IN PUCHAR buff, IN INT len,
			IN ULONG type, IN USHORT protocol);

BOOLEAN WscProcessCredential(IN PRTMP_ADAPTER pAdapter, IN PUCHAR pPlainData,
			     IN INT PlainLength, IN PWSC_CTRL pWscCtrl);
...
VOID MlmeCompensateLastBeaconRxTime(IN PRTMP_ADAPTER pAd,
				IN ULONG Now);
