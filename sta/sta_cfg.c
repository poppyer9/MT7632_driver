...
INT Set_WOW_WakeOnPort_Enable(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pAd->WOW_Cfg.bWakeonportEnable == FALSE) {
		NdisZeroMemory(pAd->WOW_Cfg.TcpPortV4, SIZE_MAX_PORT_NUM_TCPV4);
		NdisZeroMemory(pAd->WOW_Cfg.TcpPortV6, SIZE_MAX_PORT_NUM_TCPV6);
		NdisZeroMemory(pAd->WOW_Cfg.UdpPortV4, SIZE_MAX_PORT_NUM_UDPV4);
		NdisZeroMemory(pAd->WOW_Cfg.UdpPortV6, SIZE_MAX_PORT_NUM_UDPV6);
...
INT Set_WOW_IP(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	NdisZeroMemory(pAd->WOW_Cfg.WowIP, 4 * sizeof(pAd->WOW_Cfg.WowIP));
		...
		IN PSTRING				arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("WOW Usb2 Disconnect Mode = %d\n", pAd->dp_ctrl.nUsb2DisconMode));
